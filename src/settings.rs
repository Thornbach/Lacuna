...
/// Few-shot head decision threshold. 0.85 on every backend.
///
/// A CPU-only 0.60 was tried and REVERTED with the 256 resolution it existed to
/// compensate for (see `worker::default_dino_res`). Relaxing tau only made sense
/// while CPU ran out of the head's training domain and lost per-family recall;
/// back at 512 the head is in-domain and 0.85 is simply the calibrated value.
pub fn default_head_tau() -> f32 { 0.85 }
